<template>
    <div>
        <button
            class="flex justify-center mt-16 mr-auto ml-auto w-80 py-4 px-4 w-24 border-2 border-blue-700 border-opacity-0 hover:border-blue-300 hover:bg-green-600 text-center text-white text-3xl font-bold hover:text-white rounded-xl"
            type="button" :disabled="spinning" @click="updateRates()">UPDATE RATES
        </button>
        <spinner class="flex justify-center mt-8" v-if="spinning"></spinner>
        <div class="flex justify-center mt-8 opacity-0" v-else>Spinner replacement text<br/>Spinner replacement
            text<br/>Spinner replacement text
        </div>
                ...
                <tbody>
                    <tr>
                        <td class="bg-yellow-100 rounded focus-within:outline-none mb-0.5">
                            <select class="bg-yellow-100 w-full h-full text-2xl" id="cur1" :disabled="spinning"
                                    required>
                                <option disabled selected value>--SELECT--</option>
                                <option v-for="record in currency" :key="record.id" :value="record.symbol">
                                    {{ record.symbol }}
                                </option>
                            </select>
                        </td>
                        <td class="bg-yellow-400 hover:bg-yellow-500 px-5 text-2xl rounded">
                            <button class="h-10 fas" type="button" :disabled="spinning" @click="convert('left')">
                                &#xf100;
                            </button>
                        </td>
                        <td class="">
                            <input id="number" class="h-10 w-80 text-2xl text-center" type="text" :disabled="spinning"
                                   placeholder="Enter number..." :value="total">
                        </td>
                        <td class="bg-yellow-400 hover:bg-yellow-500 px-5 text-2xl rounded">
                            <button class="h-10 fas" type="button" :disabled="spinning" @click="convert('right')">
                                &#xf101;
                            </button>
                        </td>
                        <td class="bg-yellow-100 rounded focus-within:outline-none mb-0.5">
                            <select class="bg-yellow-100 w-full h-full text-2xl" id="cur2" :disabled="spinning"
                                    required>
                                <option disabled selected value>--SELECT--</option>
                                <option v-for="record in currency" :key="record.id" :value="record.symbol">
                                    {{ record.symbol }}
